...
for party_number in range(2, 101):
    ...
    n = 2048
    k, l = 2, 1
    sigma = 3  # Standard deviation for Gaussian sample
    ...
    eta = 5
    N = 2048
    alpha = 11*party_number*50  # Example value for alpha
    ...
    def rejection_sample(csn_list, zn_list):
        ...
        for csn, zn in zip(csn_list, zn_list):
            ...
            # 比率を計算
            ratio = Ds(zn_vec, s, rohs_rm) / (M * Dcsn_s(csn_vec, zn_vec, s, rohcsn_s_rm))
...
# 実行時間をCSVファイルに保存
with open('N2048_execution_times.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['party_number', 'execution_time'])
...
plt.title('Execution Times for DS2 Protocol')
plt.show()